...
def _load_bundle_cached_wind(path_str: str, mtime: float) -> dict:
    with gzip.open(path_str, "rb") as f:
        bundle = pickle.load(f)
    risk = {}
    for key, df in bundle["risk_tables"].items():
        if "limit_ms" not in df.columns and "limit_K" in df.columns:
            df = df.rename(columns={"limit_K": "limit_ms"}).drop(
                columns=["limit_C"], errors="ignore"
            )
        risk[key] = df
    bundle["risk_tables"] = risk
    h = bundle["hist"]
    bundle["hist"] = {
        "lines": h["lines"],
        "combos": h["combos"],
...
def _load_bundle() -> dict:
    ...
    return _load_bundle_cached_wind(str(BUNDLE_FILE), BUNDLE_FILE.stat().st_mtime)
